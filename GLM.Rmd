...
Margins works also with predictions: 


```{r}
effects_logit_participation_prediction1 <- margins::margins(logitres, at = list(kidslt6 = 3)) # marginal effect if everyone had 3 kids which are younger than 6 years.
```

```{r}
summary(effects_logit_participation_prediction1)
```

```{r}
effects_logit_participation_prediction2 <- margins::margins(logitres, at = list(kidslt6 = 3, kidsge6 = 3)) # marginal effect if everyone had 3 kids which are younger than 6 years and 3 kids greater equal 6 years old.
```
...
We selected the group who did not attend a college to be our base group, so that the parameters $\beta_{11} = \beta{21} = 0$.
...
```{r}
margins::margins(nels.multinom, vce = "none")
```
...
```{r}
predi1 <-prediction::prediction(nels.multinom, data = (newdat))
```




Choosing no college is 0.1810 for the student with median grades, but this probability is reduced to 0.0178 for students with top grades. Similarly, the probability of choosing a two-year school is 0.2856 for the average student but is 0.0966 for the better student. 
...
```{r}
# OLS: log wage regression on observed data (LF participants only)
wage.lm_obs <- lm(log(wage)~educ+exper, 
...
wage.heckit <- sampleSelection::selection(inlf~age+educ+I(kidslt6+kidsge6)+mtr, # modelling the selection
                                          log(wage)~educ+exper, # outcome equation
                                          data=mroz, method="ml")
...
I don't know if marginal effects can be calculated directly from the sampleSelection object. Calculation marginal for the model selection separately can be easily done though. 

```{r}
mroz$allkids <- mroz$kidslt6 + mroz$kidsge6 # I(kidslt6+kidsge6) is not a term that can be "broomed"
selection_probit <- glm(inlf~age+educ+allkids+mtr, family = binomial(link = probit), data = mroz)
summary(selection_probit)
```

```{r}
summary(margins(selection_probit))
```


```{r}
cplot(selection_probit, "allkids")
```
