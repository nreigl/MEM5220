...
Alternatively use the `poly()` function. Be careful of the additional argument `raw`^[The `poly()` argument also requires complete observations. You can use `raw` argument to estimate the term with missing observations but be aware of the consequences!].
...
2. Cp. married males make `r scales::percent(marriedmale)` (` scales::percent(marriedmale)`) more than single males.
3. Cp. a single female makes `r scales::percent(singlefemale)` (`scales::percent(singlefemale)`) less than the reference group.
3. Married females make `r scales::percent(abs(marriedfemale) - abs(married))` (`scales::percent(abs(marriedfemale) - abs(married))`) less than single females.
...
To see this let us run a simulation ^[This simulation is based on an example from Ed Rubin's [https://raw.githack.com/edrubin/EC421S20/master/notes/05-heteroskedasticity/05-heteroskedasticity.html#118](EC421S20) course.]
...
We estimate conditional quantile regressions, in the range of the 25%, 50% and 75% quantile (not that this remains linear). Standard errors are obtained via bootstrapping ^[The quantile regression subsection is adapted from [Fan Wang lecture notes](https://github.com/FanWangEcon/R4Econ)].
...
Note for example that the sign of disp reverses at the 75th quantile compared with the OLS estimate. Also note that there seems to be a gradient at the quantile hp coefficients as well as disp. 
