...
Alternative you can use the plm package. 

```{r}
data("SeatBelt", package = "pder")
SeatBelt$occfat <- with(SeatBelt, log(farsocc / (vmtrural + vmturban)))
fe1 <- plm::plm(occfat ~ log(usage) + log(percapin) + log(unemp) + log(meanage) + 
           ...
           effect = "time")
fe2 <- update(ols, effect = "twoways")
ivfe <- update(fe2, . ~ . |  . - log(usage) + ds + dp +dsp)
```

```{r}
rbind(ols = coef(summary(ols))[1,],
      fe2 = coef(summary(fe2))[1, ],
