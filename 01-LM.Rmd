...
Deviding the dependent variable by 16 changes all coefficients by the same facor $\frac{1}{16}$ and deviding the regressor by 20 changes its coefficients by the factor 20. Other statistics like $R^2$ are uneffected. 
...
Recall that R reads **x1**x**x2** as $y ~ x_1+x_2+x_1x_2$ and **x_1:x_2** as $y ~ x_1x_2$.
...
On average a woman makes \$ `r -1 * round(coefficients(lm_wage1)[[2]])` per less than a man with the *same* education, experience, and tenure. 
...
A: 
 ...
 2. Cp. married males make `r percent(marriedmale)` more than single males. 
 3. Cp. a single female makes `r percent(singlefemale)` less than the reference group. 
 3. Married females make `r percent(abs(marriedfemale) - abs(married))` less than single females. 
...
Another way of dealing with heteroskedasticity is to use the lmrob() function from the {robustbase} package^[This example has been adapted from the blogpost of @Rodrigues]. This package is quite interesting, and offers quite a lot of functions for robust linear, and nonlinear, regression models. Running a robust linear regression is just the same as with lm():
...
Following Wooldrige, we assume that the variance is proportional to the income variable *inc.*. Therefore, the optimal weight is  $\frac{1}{inc}$ hwich is given as **weight** in the **lm** call. 
...
In R, general quantile regression (and LAD as the default special case) can easily be implemented with the command **rq** from the **quantreg** package. 
